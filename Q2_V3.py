...
speaker = Dense(16)(speaker)
speaker_model = Model(inputs=brain_input, outputs=speaker)

brain_comb = Concatenate(axis=1)([brain_model.output, speaker_model.output])
brain_total = Reshape((16,2))(brain_comb)
brain_total = LSTM(16)(brain_total)
brain_total_model = Model(inputs=[brain_input, speaker_input], outputs=brain_total)

info = Dense(4)(info_input)
info_model = Model(inputs=info_input, outputs=info)

combined = Concatenate(axis=1)([brain_total_model.output, speaker_model.output])
...
history = model.fit(x=[train_brain_cn, train_memory, train_speaker], y=train_labels, epochs=50, batch_size=1, validation_split=0.3,
...
df.to_excel(f'result/model_info_v3_acc_{int(scores[1] * 100)}_loss_{int(scores[0])}.xlsx')

model.save(f'models/model_v3_acc_{int(scores[1] * 100)}_loss_{int(scores[0])}.h5')
